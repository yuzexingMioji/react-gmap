...
var InfoMap = function (_Component) {
  ...
  InfoMap.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
    ...
    if ((0, _lodash.isEqual)(nWhole, whole) && (0, _lodash.isEqual)(nSelected, selected)) {
    ...
    this.markers.map(function (oldMarker) {
      ...
      if (stay) {
        ...
        oldMarker.marker.setMap(_this3.map);
        oldMarker.label.open(_this3.map, oldMarker.marker);
      } else {
        // marker被移除了
    ...
    newMarkers.map(function (newMarker) {
      ...
      if (!marker) {
        ...
        if (!newMarker.marker && !newMarker.label) {
          ...
          newMarker.marker.setMap(_this3.map);
          newMarker.label.open(_this3.map, newMarker.marker);
        }
  ...
  InfoMap.prototype.createContent = function createContent(data) {
    ...
    var className = custom == 3 ? 'gmap-info-map-first-title-siyou' : 'noIcon';
