...
var InfoMap = function (_Component) {
  ...
  InfoMap.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
    ...
    if ((0, _lodash.isEqual)(whole, nWhole)) {
      var _ret = function () {
        // 整体不变 无须刷新地图
        if ((0, _lodash.isEqual)(selected, nSelected)) {
          return {
            v: void 0
          };
        }
        _this3.markers.map(function (m) {

          var select = nSelected.find(function (s) {
            return s.id == m.id;
          });
          if (select && !m.selected) {
            // 改变状态
            _this3.initSelectState(m.marker, true);
            m.selected = true;
          }
          // 从已选列表中移除的 也需要重置
          if (!select && m.selected) {
            _this3.initSelectState(m.marker, false, m.id);
            m.selected = false;
          }
        });
  ...
  InfoMap.prototype.updateMap = function updateMap() {
    this.initLine();
    this.initMarker();
    this.initMapLens();
  };
  ...
  InfoMap.prototype.initMarker = function initMarker() {
    ...
    whole && whole.map(function (data) {
      ...
      var _addMarkerWithInfoBub = _this4.addMarkerWithInfoBubble(mData),
          marker = _addMarkerWithInfoBub.marker,
    ...
    selected.map(function (select) {
      if (_this4.markers.findIndex(function (m) {
        return m.id == select.id;
      }) < 0) {
        _this4.markers.push(select);
      }
    });
  };
  ...
  InfoMap.prototype.initMapLens = function initMapLens(allCoords) {
    if (!allCoords) {
  ...
  InfoMap.prototype.createContent = function createContent(data) {
    ...
    content.innerHTML = "<img class='map-img' src=" + img + " />" + "<div class='map-info-wrap'>" + "<span class='first-title over-hide " + style.extraCool + "'>" + first + "</span>" + "<span class='second-title over-hide " + style.extraCooler + "'>" + second + "</span>" + "<span class='third-title over-hide " + style.extraCoolest + "'>" + third + "</span>" + "</div>" + "<div class='btn-wrap' id='btn-option'>" + icon + "</div>";
    content.className = 'gmap-info-bubble-info-wrap';
    var btn = content.getElementsByClassName('btn-wrap')[0];
    btn.onclick = function () {
      return _this5.optionClick(data);
    };
    return content;
  };
  ...
  InfoMap.prototype.addMarkerWithInfoBubble = function addMarkerWithInfoBubble(data) {
    ...
    if (!this.infoBubble) {
      ...
      this.infoBubble.bubble_.addEventListener('mouseenter', function () {
        _this6.hoverState = true;
      });

      this.infoBubble.bubble_.addEventListener('mouseleave', function () {
        _this6.hoverState = false;
        _this6.leaveTimer = setTimeout(function () {
          if (_this6.infoBubble.isOpen() && !_this6.hoverState) {
            _this6.resetMarker(_this6.markerID);
            _this6.infoBubble.close();
          }
        }, DELAY_TIME);
      });
    }
    ...
    marker.addListener('mouseover', function (event) {
      _this6.hoverState = true;
      clearTimeout(_this6.outTimer);
      clearTimeout(_this6.leaveTimer);
      if (_this6.markerID == id) {
        return;
      }

      if (_this6.infoBubble.isOpen()) {
        _this6.resetMarker(_this6.markerID, true);
        _this6.infoBubble.close();
      }

      IB_Small.close();
      _this6.openInfoBubble(content, marker, data);
      _this6.focusMarker(data, data.selected, id);
      // event.ta.currentTarget.style.opacity="1"
      var onHover = _this6.props.onHover;
    ...
    marker.addListener('mouseout', function () {
      _this6.hoverState = false;
      _this6.outTimer = setTimeout(function () {
        _this6.resetMarker(id);
        if (_this6.infoBubble.isOpen() && !_this6.hoverState) {
          _this6.infoBubble.close();
        }
      }, DELAY_TIME);
  ...
  InfoMap.prototype.setIcon = function setIcon(icon) {
