...
var api = 'http://107.150.104.221/nd/gmap';
...
var MapSuggestion = function (_Component) {
  ...
  MapSuggestion.prototype.searchPlace = function searchPlace(val) {
    ...
    return request.post(api).set('charset', 'utf-8').send(actionQuery);
  };
  ...
  MapSuggestion.prototype.handleSearch = function handleSearch(value) {
    ...
    this.timer = setTimeout(function () {
      ...
      _this3.promise.then(function (result) {
  ...
  MapSuggestion.prototype.textSearch = function textSearch(text) {
    ...
    this.abortPromise();
    this.clearDropMenu();
    this.textSearchPromise = request.post(api).set('charset', 'utf-8').send(actionQuery);

    this.textSearchPromise.then(function (result) {
  ...
  */
  ...
  MapSuggestion.prototype.handleSelect = function handleSelect(key, option) {
    ...
    this.abortPromise();
    _pubsubJs2.default.publish('loading', { from: fetter, loading: true });
    this.detailPromise = request.post(api).set('charset', 'utf-8').send(actionQuery);

    this.detailPromise.then(function (result) {
