...
exports.noAnimationFit = noAnimationFit;
/*.
...
function loadJS(key) {
  ...
  if (window.loadPromise) {
    ...
    var loadPromise = new Promise(function (resolve, reject) {
      ...
      s.src = 'http://ditu.google.cn/maps/api/js?v=3.26&libraries=places' + key;
      s.onload = resolve;
      s.onerror = reject;
...
function fitMap(map, options, getProjection) {
...
function noAnimationFit(map, options) {
  if (!window.google) {
    return;
  }
  if (!options.coords || options.coords.length == 0) {
  ...
  var div = map.getDiv();
  var zoom = getBoundsZoomLevel(bounds, { width: div.clientWidth, height: div.clientHeight });
  map.setZoom(zoom);
